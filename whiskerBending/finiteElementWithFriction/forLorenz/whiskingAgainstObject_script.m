...
%% whisker bent by object at several azimuthal angles
...
if Go
    pole_pos_in_mm=5; % distance of pole form the face; along x- axis <-- CHANGE
    l_color = 'b';
else
    pole_pos_in_mm=8;
    l_color = 'r';
...
plot_protraction = 0; % determines what protraction parameter is plotted; 1 == distance (this is for the first  submission of Lorenz' paper)
...
r0=[ 0 pole_pos_in_mm/1000]'; % pole pos in meters (x, y) <<<<<<<<<<<<<<<<<<<<<<Changed
tolerance=0.000001; % this is a weird paramter - if smaller the fminsearch doesn't work; if larger, results are noisier as expected
...
% --------------setting whisker parameters - rat whiskers 
...
ha=gca;
axis equal
%axes('XLim', [-1 wh.L*1000 ], 'YLim', [-1 wh.L*1000 ], 'YDir', 'reverse');
set(ha, 'YDir', 'reverse');
...
initialCondition = [pole_pos_in_mm/1000 1/30000]; % [location of force along the whisker (m), force acting on the whisker (N)], initial condition
...
for j=1:numPosForces
    ...
    if fval < tolerance*100 % Kluge; I don't understand why fminsearch 'converges' even though the 'tolerance' is never reached
        ...
        axes(ha);
        line(squeeze(contours(j+numZeroForces, 1, :)), squeeze(contours(j+numZeroForces, 2, :)), 'Color', 'r', 'LineWidth', 1.0);
    else
        wh.f_axial=0;
        wh.moment=0;
    end
    ...
    f_tot=[f_tot wh.f_tot];
    f_friction=[f_friction wh.f_friction];

end
%toc

%intendedAnglesAtBases=anglesAtBases-(180/pi)*moment/kTheta; % correct for compliance in the whisker pad
intendedAnglesAtBases=anglesAtBases; % turn off correction for compliance in the whisker pad

% if whisker slipped, plot whisker
ind=find(f_axial==0);
if ~isempty(ind)
    startind=min(ind)-1;
    for i=1:length(ind)
        intendedAnglesAtBases(startind+i)=intendedAnglesAtBases(startind+i-1)+incr;
        contours(startind+i+numZeroForces, 1, :)=segment*((1:(n_segments+1))-1)*sind(intendedAnglesAtBases(startind+i))*1000;
        contours(startind+i+numZeroForces, 2, :)=-segment*((1:(n_segments+1))-1)*cosd(intendedAnglesAtBases(startind+i))*1000;
        line(squeeze(contours(startind+i+numZeroForces, 1, :)), -squeeze(contours(startind+i+numZeroForces, 2, :)), 'Color', 'b', 'LineWidth', 1.0);
        %startind+i
    end
end
...
f_tot=[zeros(1, numZeroForces)  f_tot];
f_friction=[zeros(1, numZeroForces)  f_friction];

% plot whiskers for positions that don't touch whiskers
intendedAnglesAtBases=[(-(numZeroForces-1)*incr):incr:0 intendedAnglesAtBases];
for i=1:numZeroForces
    contours(i, 1, :)=segment*((1:(n_segments+1))-1)*sind(intendedAnglesAtBases(i))*1000;
    contours(i, 2, :)=-segment*((1:(n_segments+1))-1)*cosd(intendedAnglesAtBases(i))*1000;
    line(squeeze(contours(i, 1, :)), -squeeze(contours(i, 2, :)), 'Color', 'b', 'LineWidth', 1.0);
    %plot(squeeze(contours(i, 1, :)), -squeeze(contours(i, 2, :)), 'b', 'LineWidth', 1.0);
end

if plot_protraction ==1
    protractionParameter=5*tand(intendedAnglesAtBases);
    x_label = 'Protraction (mm)';
else
    protractionParameter=intendedAnglesAtBases;
    x_label = 'Theta at base (degrees)';
end
...
line(protractionParameter, moment, 'Linewidth', 1.5, 'Color', l_color)
...
line(protractionParameter, f_axial, 'Linewidth', 1.5, 'Color', l_color)

subplot(3, 2, 3); cla
set(gca, 'Linewidth', 1.5, 'FontSize', 16)
xlabel(x_label)
ylabel('Total force (N)')
line(protractionParameter, f_tot, 'Linewidth', 1.5, 'Color', l_color)

subplot(3, 2, 4); cla
set(gca, 'Linewidth', 1.5, 'FontSize', 16)
xlabel(x_label)
ylabel('Lateral force (N)')
line(protractionParameter, sqrt(f_tot.^2-f_axial.^2), 'Linewidth', 1.5, 'Color', l_color)
...
%set(gca, 'YDir', 'reverse')
line(moment, f_axial, 'Linewidth', 1.5, 'Color', l_color)
...
%set(gca, 'YDir', 'reverse')
line(sqrt(f_tot.^2-f_axial.^2), f_axial, 'Linewidth', 1.5, 'Color', l_color)
