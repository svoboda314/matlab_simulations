...
% force applied normal to the whisker; temp(2); by definition this is in the -x direction 
...
%f_pole=[temp(2) wh.friction*temp(2) 0]; % <<<<<<<<<<<NEW
f_pole=[wh.friction*temp(2) temp(2) 0];

n_segments=length(wh.I);
segment=wh.L/n_segments; % segment length in meters; total length is n_segments * segment

% initialize arrays
phi=zeros(1, n_segments); % angles of whisker segments wrt to y-axis
...
for k=2:(force_index-1) 
    ...
    moment=-(r(1)*f_pole(2)-r(2)*f_pole(1));% <<<<<<<<<<<Check sign
    ...
    %x(force_index-k)=x(force_index-k+1)-sin(phi(force_index-k))*segment;
    %y(force_index-k)=y(force_index-k+1)-cos(phi(force_index-k))*segment;
...
x(2:end)=cumsum(sin(phi)*segment);
y(2:end)=cumsum(cos(phi)*segment);
