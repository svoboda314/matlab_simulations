...
function [polePosError, varargout]=bendingRods5(params, wh, varargin) 
%
% Force (parametrized by f_pole) is applied at position params(1) along the whisker 
% Calculates the contours of the whisker 
%
% INPUTS: 
...
f_pole=[-params(2) wh.friction*temp(2) 0]; % force vector in x-y plane
...
for k=2:(force_index-1) 
    ...
    %x(force_index-k)=x(force_index-k+1)-cos(theta(force_index-k))*segment;
    %y(force_index-k)=y(force_index-k+1)-sin(theta(force_index-k))*segment;
...
wh.f_axial=dot([sin(theta(1)) cos(theta(1)) 0], f_pole); % ALREADY NEW COORDINATE SYSTEM
