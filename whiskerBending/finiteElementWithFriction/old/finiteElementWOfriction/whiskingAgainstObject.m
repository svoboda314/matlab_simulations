...
for j=1:numPosForces
    ...
    [f_fit,fval,exitflag,output] = fminsearch(@(f_fit) bendingRods5(f_fit, wh, a),initialCondition, options); 
    
    if fval < tolerance*100 % Kluge; I don't understand why fminsearch 'converges' even though the 'tolerance' is never reached
        [temp, wh]=bendingRods5(f_fit, wh);
        for i=1:size(wh.x, 2)
            temp=[wh.x(i) wh.y(i)]';
            temp=rotTheta0*temp;
            wh.x(i)=temp(1);
...
ylabel('axial_force (N)')
xlabel('moment (Nm)')
line(moment, f_axial, 'Marker', 'o')

subplot(2, 2, 4); cla
xlabel('whisker angle at follicle (degrees)')
ylabel('total_force (N)')
line(intendedAnglesAtBases, f_tot, 'Marker', 'o')
