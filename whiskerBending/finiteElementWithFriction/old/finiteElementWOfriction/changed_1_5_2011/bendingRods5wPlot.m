...
function wh=bendingRods5wPlot(temp, wh)
%
% INPUTS: 
...
%f_pole=twmp(2); % force that is applied at s_pole
...
% 
% %  compute the moment
% gamma=atan((wh.y(force_index)-wh.y(1))/(wh.x(force_index)- wh.x(1)));% FIX
% r=sqrt(((wh.y(force_index)-wh.y(1))^2+(wh.x(force_index)-wh.x(1))^2));% FIX
% beta=abs(wh.phi(force_index)-pi/2-gamma);% FIX
% wh.moment=r*f_pole*sin(beta);% FIX
