...
tolerance=0.0000001; % this is a weird paramter - if smaller the fminsearch doesn't work; if larger, results are noisier as expected
...
for j=1:numPosForces
    ...
    [x,fval,exitflag,output] = fminsearch(@(x) bendingRods6(x, a, wh),initialCondition, options); % x = [s_pole force]
%j    
    if fval < tolerance*100 % Kluge; I don't understand why fminsearch 'converges' even though the 'tolerance' is never reached
        wh=bendingRods6wPlot(x, wh);
        for i=1:size(wh.x, 2)
            temp=[wh.x(i) wh.y(i)]';
            temp=rotTheta0*temp;
            wh.x(i)=temp(1);
