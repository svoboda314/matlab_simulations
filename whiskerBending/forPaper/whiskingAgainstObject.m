...
n_segments=1000; % number of finite elements
...
incr=1.0; % increment of intended angle, degrees
thetaMin=incr; % min angle with positive force
...
conical=1; % whisker has conical shape
wh.ym=5e9; % young's modulus in pascal
base_radius= 33.5e-6; % in meters
wh.L = 16e-3; % whisker length in meters
...
line(intendedAnglesAtBases, moment,'Color','r')
...
xlim([-20 30])
set(gca, 'xtick',[-20 : 10 : 30])
line(intendedAnglesAtBases, f_axial,'Color','r')

subplot(2, 2, 3); 
ylabel('axial_force (N)')
xlabel('moment (Nm)')
line(moment, f_axial, 'Color','r')
...
xlim([-20 30])
set(gca, 'xtick',[-20 : 10 : 30])
line(intendedAnglesAtBases, f_axial,'Color','b')

subplot(2, 2, 3); 
ylabel('axial_force (N)')
xlabel('moment (Nm)')
line(moment, f_axial, 'Color','b')
