...
vpm_l4_dge=0.04; % Beierlein; could be smaller
vpm_l4_dt=0.001; % in seconds; delay of VPM-l4 compared to VPM-fs; 'models' the faster rise-time of the conductance onto fs cells (Cruikshank and Connors)

ratio_fs_l4=4; % raio of conductance fs/l4; Cruikshank and Connors
vpm_fs_cp=0.75; 
vpm_fs_dge=vpm_l4_dge*ratio_fs_l4; 

% END TO BE CHANGED

duration = .5; % duration of simulation; in seconds
...
net.vpm.sr=50; % mean spike rate
net.vpm.spikes=zeros(net.vpm.n, tsteps);% initialize spike array; zero == no spikes; one == spike

% generate VPM activity ...
tt=0:(duration*1000); % in ms 
touch_time=300;
...
for i=1:net.vpm.n
    ...
    net.vpm.spikes(i, (1/tstep)*setdiff(net.vpm.st{i}, [0]))=1;
end
...
net.l4.vm=zeros(net.l4.n, tsteps)+net.l4.el; % in vm
net.l4.gi=zeros(net.l4.n, tsteps); % in CHECK
net.l4.ge=zeros(net.l4.n, tsteps); % in CHECK
...
net.fs.vm=zeros(net.fs.n, tsteps)+net.fs.el;
net.fs.gi=zeros(net.fs.n, tsteps);
net.fs.ge=zeros(net.fs.n, tsteps);
...
net.vpm_l4.dt=vpm_l4_dt; % models the fact that vpm-l4 synapses are slower than vpm-fs synapses
...
for t=2:tsteps
    % UPDATE net.l4.vm
    net.l4.vm(:,t)=net.l4.vm(:,t-1)+(net.l4.el-net.l4.vm(:,t-1))*tstep/net.l4.tau - ...
        net.l4.gi(:,t-1).*(net.l4.vm(:,t-1) - net.l4.ei)*tstep/net.l4.tau - ...
        net.l4.ge(:,t-1).*(net.l4.vm(:,t-1) - net.l4.ee)*tstep/net.l4.tau; 
    % UPDATE net.l4.gi
    net.l4.gi(:,t)=net.l4.gi(:,t-1)*(1-tstep/net.l4.tgi)+net.fs_l4.dgi*net.fs_l4.cm*net.fs.spikes(:, t-1);
    % UPDATE net.l4.ge
    net.l4.ge(:,t)=net.l4.ge(:,t-1)*(1-tstep/net.l4.tge)+net.l4_l4.dge*net.l4_l4.cm*net.l4.spikes(:, t-1)+...
        net.vpm_l4.dge*net.vpm_l4.cm*net.vpm.spikes(:, t-1);
    % UPDATE net.fs.vm
    net.fs.vm(:,t)=net.fs.vm(:,t-1)+(net.fs.el-net.fs.vm(:,t-1))*tstep/net.fs.tau - ...
        net.fs.gi(:,t-1).*(net.fs.vm(:,t-1) - net.fs.ei)*tstep/net.fs.tau - ...
        net.fs.ge(:,t-1).*(net.fs.vm(:,t-1) - net.fs.ee)*tstep/net.fs.tau; 
    % UPDATE net.fs.gi
    net.fs.gi(:,t)=net.fs.gi(:,t-1)*(1-tstep/net.fs.tgi)+net.fs_fs.dgi*net.fs_fs.cm*net.fs.spikes(:, t-1);
    % UPDATE net.fs.ge
    net.fs.ge(:,t)=net.fs.ge(:,t-1)*(1-tstep/net.fs.tge)+net.l4_fs.dge*net.l4_fs.cm*net.l4.spikes(:, t-1)+...
...
% 1 ms bins
l4=sum(reshape(mean(net.l4.spikes(:,1:(tsteps-1))), round(1/tstep), round(tstep*tsteps))); 
tt=1:length(l4);
fs=sum(reshape(mean(net.fs.spikes(:,1:(tsteps-1))), round(1/tstep), round(tstep*tsteps))); 
vpm=sum(reshape(mean(net.vpm.spikes(:,1:(tsteps-1))), round(1/tstep), round(tstep*tsteps))); 
...
dd=std(ge(1, :));
for i=1:nn
line(times,ge(i, :)+dd*(i-1))
end
%plot(ge(1, :))
subplot(3,1,3)
dd=std(gi(1, :));
for i=1:nn
line(times,gi(i, :)+dd*(i-1))
end
%plot(gi(1, :))

%% various stats

vpm_sr = mean(mean(net.vpm.spikes))*tsteps
l4_sr = mean(mean(net.l4.spikes))*tsteps
fs_sr = mean(mean(net.fs.spikes))*tsteps
